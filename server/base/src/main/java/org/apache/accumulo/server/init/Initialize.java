...
public class Initialize implements KeywordExecutable {
  ...
  static void checkInit(ZooReaderWriter zoo, VolumeManager fs, InitialConfiguration initConfig,
    ...
    if (isInitialized(fs, initConfig)) {
      if (opts.clearInstanceName || opts.forceResetSecurity) {
        log.warn("Filesystem appears initialized but --clear-instance-name or --force was set."
            + " Removing existing initialization data from filesystem.");
        for (String baseDir : initConfig.getVolumeUris()) {
          Path instanceIdPath = new Path(baseDir, Constants.INSTANCE_ID_DIR);
          Path versionPath = new Path(baseDir, Constants.VERSION_DIR);
          try {
            if (fs.exists(instanceIdPath)) {
              fs.deleteRecursively(instanceIdPath);
              log.info("Removed {}", instanceIdPath);
            }
            if (fs.exists(versionPath)) {
              fs.deleteRecursively(versionPath);
              log.info("Removed {}", versionPath);
            }
          } catch (Exception e) {
            log.warn("Failed to clean filesystem: {}", e.getMessage());
          }
        }
      } else {
  ...
  private boolean doInit(ZooReaderWriter zoo, Opts opts, VolumeManager fs,
    ...
    try {
      checkInit(zoo, fs, initConfig, opts);
