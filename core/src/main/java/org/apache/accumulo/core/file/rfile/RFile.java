...
public class RFile {
  ...
  public static class Writer implements FileSKVWriter {
    ...
    private List<float[]> currentBlockVectors;
    private List<List<float[]>> allBlockVectors; // All vectors per block for tessellation footer
    private int vectorDimension = -1;
    private long currentVectorBlockOffset = -1;
    ...
    public Writer(BCFile.Writer bfw, int blockSize, int indexBlockSize,
      ...
      this.sampler = sampler;
      this.currentBlockVectors = new ArrayList<>();
      this.allBlockVectors = new ArrayList<>();
    }
    ...
    public synchronized void close() throws IOException {
      ...
      // Flush any remaining vectors in the current block
      flushVectorBlock();

      // Write vector index if present
      if (vectorIndex != null && !vectorIndex.getBlocks().isEmpty()) {
        mba.writeBoolean(true); // Vector index present
        vectorIndex.write(mba);

        // Build and write tessellation footer for data-drift-resistant indexing
        if (!allBlockVectors.isEmpty() && vectorDimension > 0) {
          VectorIndexFooter tessFooter =
              new VectorIndexFooter(vectorDimension, VectorIndexFooter.IndexingType.TESSELLATION);
          tessFooter.buildTessellationIndex(allBlockVectors,
              org.apache.accumulo.core.graph.SphericalTessellation.DEFAULT_DEPTH);
          mba.writeBoolean(true); // Tessellation footer present
          tessFooter.write(mba);
          log.debug("Wrote tessellation index footer: {} blocks, depth {}", allBlockVectors.size(),
              org.apache.accumulo.core.graph.SphericalTessellation.DEFAULT_DEPTH);
        } else {
          mba.writeBoolean(false); // No tessellation footer
        }
      } else {
        mba.writeBoolean(false); // No vector index
      }
    ...
    public void append(Key key, Value value) throws IOException {
      ...
      // Handle vector values for index building
      if (vectorIndexEnabled && value.getValueType() == ValueType.VECTOR_FLOAT32) {
        handleVectorValue(value);
      }
    ...
    public void enableVectorIndex(int vectorDimension) {
      if (dataClosed) {
        throw new IllegalStateException("Cannot enable vector index, data closed");
      }
      this.vectorIndexEnabled = true;
     ...
     */
    public void writeVectorBlock(List<KeyValue> vectorData) throws IOException {
      if (dataClosed) {
        throw new IllegalStateException("Cannot write vector block, data closed");
      }
      ...
      // Validate and extract vectors for centroid calculation
      List<float[]> vectors = new ArrayList<>();
      for (KeyValue kv : vectorData) {
        if (kv.getValue().getValueType() != ValueType.VECTOR_FLOAT32) {
          throw new IllegalArgumentException("All values must be VECTOR_FLOAT32 type");
        }
        float[] vector = kv.getValue().asVector();
      ...
      // Calculate block centroid for vector index
      float[] centroid = calculateCentroid(vectors);
      long blockStartOffset = getCurrentBlockOffset();

      // Write the actual data
      for (KeyValue kv : vectorData) {
        lgWriter.append(kv.getKey(), kv.getValue());
      }

      // Record vector block metadata
    ...
    /**
     * Handles individual vector values for index building. Tracks vectors per block and commits
     * block metadata when a block boundary is reached (based on block size threshold).
     */
    private void handleVectorValue(Value value) throws IOException {
      ...
      if (currentVectorBlockOffset == -1) {
        currentVectorBlockOffset = getCurrentBlockOffset();
      }

      currentBlockVectors.add(vector);

      // Flush block metadata when we accumulate enough vectors
      // (aligned with RFile data block boundaries)
      if (currentBlockVectors.size() >= 1000) {
        flushVectorBlock();
      }
    }

    /**
     * Commits the current vector block's metadata to the index and tessellation footer.
     */
    private void flushVectorBlock() {
      if (currentBlockVectors.isEmpty()) {
        return;
      }

      float[] centroid = calculateCentroid(currentBlockVectors);
      long blockEndOffset = getCurrentBlockOffset();
      int blockSizeBytes = (int) (blockEndOffset - currentVectorBlockOffset);

      if (vectorIndex != null) {
        VectorIndex.VectorBlockMetadata blockMetadata = new VectorIndex.VectorBlockMetadata(
            centroid, currentBlockVectors.size(), currentVectorBlockOffset, blockSizeBytes);
        vectorIndex.addBlock(blockMetadata);
      }

      // Save for tessellation footer (needs all vectors, not just centroids)
      allBlockVectors.add(new ArrayList<>(currentBlockVectors));

      currentBlockVectors.clear();
      currentVectorBlockOffset = -1;
    }
    ...
    private long getCurrentBlockOffset() {
      ...
      // For now, return file length as an approximation
      return fileWriter.getLength();
    }
  }
  ...
  public static final class Reader extends HeapIterator implements RFileSKVIterator {
    ...
    // Vector support fields
    private VectorIndex vectorIndex;
    private VectorIndexFooter tessellationFooter;
    ...
    public Reader(CachableBlockFile.Reader rdr) throws IOException {
      ...
      try (CachableBlockFile.CachedBlockRead mb = reader.getMetaBlock("RFile.index")) {
        ...
        // Read vector index if present (only for newer versions)
        if (ver == RINDEX_VER_8 && mb.available() > 0 && mb.readBoolean()) {
          vectorIndex = new VectorIndex();
          vectorIndex.readFields(mb);

          // Read tessellation footer if present
          if (mb.available() > 0 && mb.readBoolean()) {
            tessellationFooter = new VectorIndexFooter();
            tessellationFooter.readFields(mb);
          }
        } else {
          vectorIndex = null;
          tessellationFooter = null;
        }
    ...
    /** Returns the tessellation footer if present, or null. */
    public VectorIndexFooter getTessellationFooter() {
      return tessellationFooter;
    }
    ...
    public VectorIterator createVectorIterator(float[] queryVector,
      ...
      return createVectorIterator(queryVector, similarityType, topK, threshold, null);
    }

    /**
     * Creates a new VectorIterator with authorizations for visibility filtering.
     *
     * @param queryVector the query vector for similarity search
     * @param similarityType the type of similarity computation
     * @param topK number of top results to return
     * @param threshold minimum similarity threshold
     * @param authorizations comma-separated authorizations for visibility filtering, or null
     * @return configured VectorIterator
     */
    public VectorIterator createVectorIterator(float[] queryVector,
        VectorIterator.SimilarityType similarityType, int topK, float threshold,
      ...
      if (authorizations != null) {
        options.put(VectorIterator.AUTHORIZATIONS_OPTION, authorizations);
      }

      try {
        vectorIter.init(this, options, null);
      } catch (IOException e) {
